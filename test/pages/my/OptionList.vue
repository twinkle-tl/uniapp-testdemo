<template>
	<view class="optionlist-container">
		<view class="option-panel">
            <view class="row2 member">
                <view class="title more-right">会员中心</view>
            </view>
            <view class="row2 wallet">
                <view class="title more-right">我的优惠</view>
            </view>
        </view>
        <view class="option-panel">
            <view class="row2 service">
                <view class="title more-right">服务中心</view>
            </view>
            <view class="row2 mihome">
                <view class="title more-right">小米之家</view>
            </view>
        </view>
        <view class="option-panel">
            <view class="row2 fcode">
                <view class="title more-right">我的F码</view>
            </view>
            <view class="row2 gift">
                <view class="title more-right">礼物码兑换</view>
            </view>
        </view>
        <view class="option-panel">
            <view class="row2">
                <view class="title more-right">设置</view>
            </view>
        </view>
	</view>
</template>

<script>
export default {
    data() {
        return {
            
        }
    },
    methods: {
        
    },
}
</script>

<style scoped>
    .optionlist-container {
      overflow: hidden;
    }
    .optionlist-container .option-panel {
      background: #fff;
      font-size: 30rpx;
      margin-bottom: 21rpx;
    }
    .optionlist-container .option-panel .row2 {
      height: 108rpx;
      line-height: 108rpx;
      padding-left: 116rpx;
      background: no-repeat 32rpx url("../../static/images/i-setting.fb9625b3f2.png");
      background-size: 50rpx auto;
    }
    .optionlist-container .option-panel .row2:not(:first-child) .title {
      border-top: 2rpx solid rgba(0,0,0,0.15);
    }
	.title{
		height: 108rpx;
		line-height: 108rpx;
	}
    .optionlist-container .option-panel .row2.member {
      background-image: url("data:image/png;base64,");
    }
    .optionlist-container .option-panel .row2.wallet {
      background-image: url("data:image/png;base64,");
    }
    .optionlist-container .option-panel .row2.service {
      background-image: url("data:image/png;base64,");
    }
    .optionlist-container .option-panel .row2.mihome {
      background-image: url("data:image/png;base64,");
    }
    .optionlist-container .option-panel .row2.fcode {
      background-image: url("data:image/png;base64,");
    }
    .optionlist-container .option-panel .row2.gift {
      background-image: url("https://m.mi.com/static/img/i-gift.ece9aa1222.png");
    }      
</style>
